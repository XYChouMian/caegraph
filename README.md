...
> **Status: Pre-Alpha (Phase 2 — CAE Data Pipeline, in progress).** Phase 0 (foundation) and Phase 1 (core vocabulary: `BaseObject`, registry, shared enums, logging) are complete. The architecture baseline is ADR-015~019 (CAEGraph as the canonical domain representation; topology subsystem, construction, backend-adaptation, domain-composition and entity/relation model contracts). Coding gates 1–3 plus the mesh representation builder have landed: the CAEGraph domain core (`CAEGraph`, `Field`, boundary vocabulary), the topology subsystem (`Mesh`, `CellType`) and node-graph construction with NodeCategory derivation (`caegraph.graph.MeshRepresentationBuilder`). The remaining data band (loaders, backend adapter, transforms, dataset) is in progress; GNN training utilities remain planned.
