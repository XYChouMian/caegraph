...
BaseObject validates object invariants, not metadata semantics. Metadata is an extensible annotation channel unless a subclass explicitly declares metadata constraints. Changing metadata does not imply domain-state validation; classes assigning domain semantics to metadata keys must override `on_metadata_changed`.

- Representation construction follows ADR-016 and is currently grouped in the graph layer for Phase 2 implementation; package ownership is not frozen. Representation builders map source discretizations (mesh / grid / particles) onto a `CAEGraph` (ADR-015). Mesh is one way to construct CAEGraph, not the definition of CAEGraph. Construction boundaries are defined in ADR-016 (accepted); builder APIs, class names, registry, and module layout are deliberately not frozen. The Phase 2 minimal entity/relation model of CAEGraph — node/cell entity families (node entities serving as graph vertices is a representation choice), canonical deduplicated node-pair relations, and region-driven NodeCategory limited to boundary-participation regions — is frozen by ADR-019. The topology subsystem (`Mesh`) does not provide a `to_graph()` conversion method because topology objects do not own representation construction logic — construction is handled by representation builders (ADR-016). The dependency rule remains that core never imports graph (ADR-007).
...
- **Validation layering convention (BaseObject contract)**: `validate()` is a *complete object consistency check* — it runs at construction and on explicit invocation (debug, pre-serialization checks) and composes the private layer hooks subclasses define: `_validate_state` (invariants that never read `self._metadata`), `_validate_metadata` (invariants over the annotation itself) and `_validate_cross` (invariants reading both state and metadata). `on_metadata_changed` is a *metadata mutation hook*, not a validate alias: its default implementation performs no validation (metadata is an annotation channel), and classes assigning domain semantics to metadata keys override it and define their own re-validation strategy — which layers to re-run is the override's decision (authoring guidance, not a frozen scope: invariants that read `self._metadata` belong to the re-run set, statically decidable). `update_metadata` is a transaction boundary: the candidate mapping is staged, the hook is invoked, and any exception rolls the whole batch back — the rollback is a transaction guarantee independent of validation semantics (fail-clean, not only fail-loud). Cost note: re-checking CAEGraph's edge invariants is O(E log E) (the `sorted(set(...))` re-scan) — relevant only to overriding strategies; CAEGraph assigns no metadata semantics, so its metadata path performs no validation by default. Classes whose validation is a single state-only check may keep one `validate()` without decomposition — the layer declarations apply where multiple layers exist.

| Lifecycle event | BaseObject default | Classes assigning domain semantics to metadata keys |
| --- | --- | --- |
| Construction | `validate()` — full object consistency check | `validate()` — full |
| Explicit `validate()` | full | full |
| `update_metadata()` | `on_metadata_changed()` — no-op | custom re-validation strategy defined by the override |
...
| **Phase 2 — CAE data pipeline** (current) | `caegraph.core` domain core: **CAEGraph** (canonical domain representation, ADR-015) + topology subsystem (`Mesh`, `CellType`, connectivity, facets; CAEGraph core, `Field`/boundary vocabulary, `Mesh`/`CellType` landed) + `Field`; data band geometry/io/graph/transforms/dataset: representation construction + entity/relation model (ADR-016/019), backend adapter (ADR-017, PyG), gmsh first, VTK write-back (R1) | conversion invariants validated (topology/conservation/BC mapping); PyG boundary enforced |
