...
| **Phase 2 — CAE data pipeline** (current) | `caegraph.core` domain core: **CAEGraph** (canonical domain representation, ADR-015) + topology subsystem (`Mesh`, `CellType`, connectivity, facets; CAEGraph core, `Field`/boundary vocabulary, `Mesh`/`CellType` landed) + `Field`; data band geometry/io/graph/transforms/dataset: representation construction (ADR-016), backend adapter (ADR-017, PyG), gmsh first, VTK write-back (R1) | conversion invariants validated (topology/conservation/BC mapping); PyG boundary enforced |
