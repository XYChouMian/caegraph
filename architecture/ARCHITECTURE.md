...
BaseObject validates object invariants, not metadata semantics. Metadata is an extensible annotation channel unless a subclass explicitly declares metadata constraints.
...
- **Validation layering convention (BaseObject contract)**: subclasses decompose validation into private layer hooks — `_validate_state` (invariants that never read `self._metadata`), `_validate_metadata` (invariants over the annotation itself) and `_validate_cross` (invariants reading both state and metadata); the public entry `validate()` composes all defined layers and runs at construction, after every metadata update, and on explicit re-check. Two rules govern the design. The **dependency rule**: after a metadata update, every invariant that reads metadata (metadata and cross layers) must re-run, while state layers may be skipped — the criterion is statically decidable (whether the invariant reads `self._metadata`). The **write rule**: every mutable write stages first, validates, and rolls back on failure (fail-clean, not only fail-loud; `BaseObject.update_metadata` implements this atomically). The hook name `on_metadata_changed` refers to the *trigger cause*, not the validation scope: its default implementation runs the full `validate()` (including the state layer), and precision triggering is obtained only by explicit subclass override. Cost note: re-checking CAEGraph's edge invariants is O(E log E) (the `sorted(set(...))` re-scan), so the default full re-run is a correctness-first choice, not a zero-cost one; precision triggering is deliberately deferred until (a) the first subclass that assigns domain semantics to metadata keys exists, or (b) large graphs see high-frequency metadata updates.
