...
CAEGraph 依赖 PyTorch 生态，CUDA 与 BLAS 栈对安装方式敏感。团队与 Agent 需要共享唯一、可复现的开发环境，同时发布包需要明确最低 Python 版本并持续验证兼容范围。环境规则还必须区分运行时依赖、开发工具和文档工具，避免声明漂移。

项目早期曾先以 Python 3.11 为标准环境，再通过 CI 引入 Python 3.10 兼容性，最终决定将本地标准环境统一到 Python 3.10。该演化曾记录在 ADR-005、ADR-006；Phase 0 整理后，本 ADR 吸收其最终有效决策，过程仍可从 Git 历史追溯。
