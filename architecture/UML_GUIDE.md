...
- Phase 2 is in progress. The design UML defines the principal abstractions per ADR-015 (`CAEGraph` as the canonical domain representation, `Mesh` (topology subsystem), `Field`, `CAEDataset`, `Model`, plus the backend adapter) and the ADR-008 workflow bands: geometry/io/graph/transforms/dataset (R1), physics/models/assimilation/workflow (R2+R4), inference (R3). ADR-016 (construction contract) and ADR-017 (backend adaptation contract) are proposed follow-ups extracted from ADR-015.
- ADR-009 (as amended by ADR-015) makes the conversion and inheritance boundaries explicit: representation builders own source-discretization → CAEGraph (construction boundary per ADR-016, proposed); the backend adapter owns CAEGraph → PyG Data (adaptation boundary per ADR-017, proposed); BaseObject covers the domain object family (CAEGraph, Mesh, Field); CAEDataset/Model remain backend-specific (PyG Dataset / torch.nn.Module are the Phase 2 choices).
