...
- Phase 2 is in progress. The design UML defines the principal abstractions per ADR-015 (`CAEGraph` as the canonical domain representation, `Mesh` (topology subsystem), `Field`, `CAEDataset`, `Model`, plus the backend adapter) and the ADR-008 workflow bands: geometry/io/graph/transforms/dataset (R1), physics/models/assimilation/workflow (R2+R4), inference (R3). ADR-016 (construction contract) and ADR-017 (backend adaptation contract) are accepted follow-ups extracted from ADR-015, alongside ADR-018 (domain composition).
- ADR-009 (as amended by ADR-015) makes the conversion and inheritance boundaries explicit: representation builders own source-discretization → CAEGraph (construction boundary per ADR-016, accepted); the backend adapter owns CAEGraph → PyG Data (adaptation boundary per ADR-017, accepted); BaseObject covers the domain object family (CAEGraph, Mesh, Field); CAEDataset/Model remain backend-specific (PyG Dataset / torch.nn.Module are the Phase 2 choices).
- Generated UML contains the implemented vocabulary: the Phase 1 classes (`BaseObject`, `Registry`, `BoundaryType`, `NodeCategory`) plus the Phase 2 core, topology and construction classes (`CAEGraph`, `Field`, `BoundaryRegion`/`BoundarySpec`/`BoundaryManager`, `CellType`, `Mesh`, `MeshRepresentationBuilder`). The remaining Phase 2 abstractions (loaders, backend adapter, transforms, dataset) live only in the design UML until they are implemented and regenerated.
