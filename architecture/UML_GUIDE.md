...
- Generated UML contains the implemented vocabulary: the Phase 1 classes (`BaseObject`, `Registry`, `BoundaryType`, `NodeCategory`) plus the Phase 2 core and topology classes (`CAEGraph`, `Field`, `BoundaryRegion`/`BoundarySpec`/`BoundaryManager`, `CellType`, `Mesh`). The remaining Phase 2 abstractions (loaders, representation builder, backend adapter, transforms, dataset) live only in the design UML until they are implemented and regenerated.
